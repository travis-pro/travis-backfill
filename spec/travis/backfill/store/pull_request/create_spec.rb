describe Travis::Backfill::Store::PullRequest::Create do
  let(:store) { described_class.new(opts) }

  before { FactoryGirl.create(:request, id: 1, event_type: :pull_request, pull_request_id: 1) }
  ...
  describe 'not given :rerun' do
    let(:opts) { {} }
    it { expect(store.ids_within(1..3)).to eq [3] }
  end
